<template>
  <Dialog v-model="dialogVisible" title="机构套餐字段权限" width="1200px">
...
const handleTableSelectionChange = (selection: any[]) => {
  ...
  selection.forEach(table => {
    if (!formData.value.tableColumnIdMap[table.id] || formData.value.tableColumnIdMap[table.id].length === 0) {
      // 如果字段已加载，立即全选
      if (allFieldIdsCache.value[table.id]) {
        formData.value.tableColumnIdMap[table.id] = [...allFieldIdsCache.value[table.id]]
      } else {
        // 如果字段未加载，标记为需要全选
        formData.value.tableColumnIdMap[table.id] = []
      }
    }
...
const applyFieldSelection = (tableId: number) => {
  if (fieldTreeRef.value) {
    fieldTreeRef.value.setCheckedKeys(formData.value.tableColumnIdMap[tableId] || [])
...
const handleTableRowClick = async (row: PermissionApi.PermissionAssignRoleTableReqVO) => {
  ...
  // 确保权限已初始化
  if (!formData.value.tableColumnIdMap[row.id]) {
    formData.value.tableColumnIdMap[row.id] = []
  }
  ...
  try {
    ...
    if (selectedTableIds.value.includes(row.id) &&
      formData.value.tableColumnIdMap[row.id].length === 0) {
      formData.value.tableColumnIdMap[row.id] = [...allLeafKeys]
    }
...
const saveCurrentTableState = () => {
  ...
  // 更新权限数据
  formData.value.tableColumnIdMap[tableId] = leafKeys as number[]
}
...
const updateTreeNodeAllState = () => {
  ...
  const checkedLeafKeys = formData.value.tableColumnIdMap[currentTable.value.id] || []
...
const updateTableSelectionImmediately = (tableId: number) => {
  // 检查该表是否有选中的字段
  const hasSelectedFields = formData.value.tableColumnIdMap[tableId] && formData.value.tableColumnIdMap[tableId].length > 0;
...
const handleFieldTreeCheck = (data: any, { checkedKeys }: { checkedKeys: number[] }) => {
  ...
  // 更新当前表的权限
  formData.value.tableColumnIdMap[currentTable.value.id] = leafKeys
...
const handleFieldTreeNodeAll = () => {
  ...
  if (fieldTreeNodeAll.value) {
    ...
    formData.value.tableColumnIdMap[currentTable.value.id] = []
...
const open = async (id?: number) => {
  ...
  try {
    ...
    // 2. 加载机构已有字段权限
    const res = await InstPackageApi.getInstPackage({ id: id!,updateMode: SystemupdateModeEnum.TABLE_COLUMN })
    // 设置选中
    formData.value = res
    // 处理权限数据
    ...
    for (const tableId in formData.value.tableColumnIdMap) {
      if (formData.value.tableColumnIdMap[tableId] && formData.value.tableColumnIdMap[tableId].length > 0) {
  ...
  try {
    ...
    for (const tableId of unloadedTables) {
      try {
        ...
        if (!formData.value.tableColumnIdMap[tableId] || formData.value.tableColumnIdMap[tableId].length === 0) {
          formData.value.tableColumnIdMap[tableId] = [...allFieldIds];
        }
    ...
    selectedTableIds.value.forEach(tableId => {
      if (formData.value.tableColumnIdMap[tableId]) {
        tableColumnIdMapToSave[tableId] = formData.value.tableColumnIdMap[tableId]
      }
    })
    ...
    const data = {
      ...
      updateMode: SystemupdateModeEnum.TABLE_COLUMN
