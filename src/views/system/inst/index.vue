<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" @selection-change="handleRowCheckboxChange">
      ...
      <el-table-column label="机构代码" align="center" prop="instId" width="185"/>
      ...
      <el-table-column label="操作" align="center" min-width="110" fixed="right">
        <template #default="scope">
          <el-button
            link
            type="primary"
            @click="openForm('update', scope.row.instId)"
            v-hasPermi="['system:inst:update']"
          >
            编辑
          </el-button>
          <el-button
            link
            type="danger"
            @click="handleDelete(scope.row.instId)"
            v-hasPermi="['system:inst:delete']"
          >
            删除
          </el-button>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <InstForm ref="formRef" @success="getList" />
</template>
...
import InstForm from './InstForm.vue'

defineOptions({ name: 'SystemInst' })
...
const packageList = ref([] as InstPackageApi.InstPackageVO[]) //机构套餐列表

/** 查询列表 */
const getList = async () => {
  loading.value = true
  try {
    const data = await InstApi.getInstPage(queryParams)
...
/** 添加/修改操作 */
const formRef = ref()
const openForm = (type: string, instId?: string) => {
  formRef.value.open(type, instId)
}
...
const handleDelete = async (instId: string) => {
  try {
    ...
    // 发起删除
    await InstApi.deleteInst(instId)
...
const handleRowCheckboxChange = (rows: InstApi.InstVO[]) => {
  checkedIds.value = rows.map((row) => row.instId)
}
...
const handleDeleteBatch = async () => {
  try {
    ...
    await message.delConfirm()
    // 发起批量删除
    await InstApi.deleteInstList(checkedIds.value)
    message.success(t('common.delSuccess'))
    // 刷新列表
    await getList()
  } catch {}
...
const handleExport = async () => {
  try {
    ...
    const data = await InstApi.exportInst(queryParams)
    download.excel(data, '机构列表.xls')
  } catch {
  } finally {
