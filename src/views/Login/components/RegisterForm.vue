<template>
  ...
  >
    <el-row style="margin-right: -10px; margin-left: -10px">
      ...
      <el-col :span="24" style="padding-right: 10px; padding-left: 10px">
        <el-form-item v-if="registerData.instEnable === 'true'" prop="instName">
          <el-input
            v-model="registerData.registerForm.instName"
            :placeholder="t('login.instname')"
...
const registerRules = {
  instName: [
    { required: true, trigger: 'blur', message: '请输入您所属的机构' },
    { min: 2, max: 20, message: '机构账号长度必须介于 2 和 20 之间', trigger: 'blur' }
  ],
  username: [
...
const registerData = reactive({
  ...
  instEnable: import.meta.env.VITE_APP_INST_ENABLE,
  registerForm: {
    instName: import.meta.env.VITE_APP_DEFAULT_LOGIN_INST || '',
    nickname: '',
    instId: 0,
    username: '',
    password: '',
...
const handleRegister = async (params: any) => {
  loading.value = true
  try {
    if (registerData.instEnable) {
      await getInstId()
      registerData.registerForm.instId = authUtil.getInstId()
    }
...
const getInstId = async () => {
  if (registerData.instEnable === 'true') {
    const res = await LoginApi.getInstIdByName(registerData.registerForm.instName)
    authUtil.setInstId(res)
  }
}

// 根据域名，获得机构信息
const getInstByWebsite = async () => {
  const website = location.host
  const res = await LoginApi.getInstByWebsite(website)
  if (res) {
    registerData.registerForm.instName = res.name
    authUtil.setInstId(res.id)
  }
}
