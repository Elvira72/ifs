<template>
  ...
  >
    <el-row style="margin-right: -10px; margin-left: -10px">
      ...
      <el-col :span="24" style="padding-right: 10px; padding-left: 10px">
        <el-form-item v-if="resetPasswordData.instEnable === 'true'" prop="instName">
          <el-input
            v-model="resetPasswordData.instName"
...
const rules = {
  instName: [{ required: true, min: 2, max: 20, trigger: 'blur', message: '长度为4到16位' }],
...
const getSmsCode = async (params) => {
  if (resetPasswordData.instEnable === 'true') {
    await getInstId()
  }
...
const getInstId = async () => {
  if (resetPasswordData.instEnable === 'true') {
    const res = await LoginApi.getInstIdByName(resetPasswordData.instName)
    if (res == null) {
      message.error(t('login.invalidInstName'))
      throw t('login.invalidInstName')
    }
